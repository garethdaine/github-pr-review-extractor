...
async function handlePostGitHubReview(request: any, sendResponse: (response: any) => void) {
  try {
    ...
    function formatInlineBody(comment: any, anchoredNote: string | null): string {
    ...
    function buildSummaryBody(params: {
      ...
      const { total, postedInline, unplaced, severityCounts } = params;
      const lines: string[] = [];
      lines.push('## 🤖 AI Code Review');
      lines.push('');
      lines.push(`- Inline comments posted: **${postedInline}** / **${total}**`);
      lines.push(`- Severity: 🔴 **${severityCounts.critical}** critical, 🟡 **${severityCounts.warning}** warning, 🔵 **${severityCounts.suggestion}** suggestion`);
      if (unplaced.length) {
        lines.push('');
        lines.push('### Unplaced Findings');
        lines.push('_Some findings could not be attached to a specific changed line in the PR diff._');
        lines.push('');
    ...
    for (let i = 0; i < (comments as any[]).length; i++) {
      ...
      let anchoredNote: string | null = null;
      ...
      if (!resolvedLine && desiredLine && desiredLine >= 1) {
        ...
        if (mappedFromPatchIndex && patchInfo.validNewLineSet.has(mappedFromPatchIndex)) {
          ...
          anchoredNote = 'Anchored from diff-line index.';
        }
      }
      ...
      if (!resolvedLine && desiredLine && desiredLine >= 1) {
        ...
        if (snapped != null) {
          ...
          anchoredNote = 'Anchored to the nearest line in the PR diff.';
        }
      }
      ...
      resolvedInline.push({
        ...
        line: resolvedLine,
        side: 'RIGHT',
        body: formatInlineBody(comment, anchoredNote)
      });
    }

...
class LLMClient {
   ...
   * @param {Array} checkTypes - Types of checks to perform
   * @returns {Promise<Object>} - Review results
   */
  async reviewCode(code, context, checkTypes) {
    const systemPrompt = this._buildSystemPrompt(checkTypes);
  ...
  /**
   * Build system prompt based on check types
   */
  _buildSystemPrompt(checkTypes) {
  ...
  /**
   * Build review prompt with code and context
   */
  _buildReviewPrompt(code, context) {
    ...
    prompt += code;
    prompt += '\n```\n\n';
    prompt += 'Analyze this code and provide your review as a JSON array of issues.';

...
function renderIssuesList(issues, filterOptions = null) {
  ...
  // Clean up previous virtual scroll if exists
  ...
  if (displayIssues.length > 100) {
    renderIssuesListVirtual(displayIssues, issues);
    return;
  }
...
function renderIssuesListVirtual(displayIssues, allIssues) {
  const issuesList = document.getElementById('issuesList');

  // Simple virtual scrolling implementation
  const ITEM_HEIGHT = 120; // Approximate height per item
  const containerHeight = Math.min(displayIssues.length * ITEM_HEIGHT, 300);

  issuesList.style.height = `${containerHeight}px`;
  issuesList.style.overflowY = 'auto';
  issuesList.style.position = 'relative';

  let visibleStart = 0;
  let visibleEnd = Math.ceil(containerHeight / ITEM_HEIGHT) + 5; // Render 5 extra items

  function renderVisibleItems() {
    const scrollTop = issuesList.scrollTop;
    visibleStart = Math.max(0, Math.floor(scrollTop / ITEM_HEIGHT) - 2);
    visibleEnd = Math.min(displayIssues.length, visibleStart + Math.ceil(containerHeight / ITEM_HEIGHT) + 4);

    // Create spacer for items before visible area
    const spacerBefore = document.createElement('div');
    spacerBefore.style.height = `${visibleStart * ITEM_HEIGHT}px`;
    spacerBefore.id = 'spacer-before';

    // Create container for visible items
    const visibleContainer = document.createElement('div');
    visibleContainer.id = 'visible-items';

    // Create spacer for items after visible area
    const spacerAfter = document.createElement('div');
    spacerAfter.style.height = `${(displayIssues.length - visibleEnd) * ITEM_HEIGHT}px`;
    spacerAfter.id = 'spacer-after';

    issuesList.innerHTML = '';
    issuesList.appendChild(spacerBefore);
    issuesList.appendChild(visibleContainer);
    issuesList.appendChild(spacerAfter);
    ...
    for (let i = visibleStart; i < visibleEnd; i++) {
      ...
      const originalIndex = allIssues.findIndex(iss =>
        iss.title === issue.title &&
        iss.filePath === issue.filePath &&
        iss.author === issue.author &&
        iss.severity === issue.severity
      );

      const issueItem = createIssueItem(issue, originalIndex >= 0 ? originalIndex : i);
      visibleContainer.appendChild(issueItem);
    }
...
function createIssueItem(issue, originalIndex) {
  const issueItem = document.createElement('div');
  issueItem.className = 'issue-item';
  issueItem.style.minHeight = '120px';
  ...
  const outdatedLabel = issue.outdated ? ' <span style="color: #57606a; font-size: 10px;">(Outdated)</span>' : '';
  const authorBadge = issue.isBot ? '🤖' : '👤';
  const confidenceBadge = issue.confidence !== undefined
    ? ` <span style="font-size: 10px; color: #57606a;">(${Math.round(issue.confidence * 100)}% confidence)</span>`
    : '';
  ...
  issueItem.innerHTML = `
    <div class="issue-header">
      <span class="severity-badge ${severityClass}" aria-label="Severity: ${issue.severity}">${severityEmoji} ${issue.severity.toUpperCase()}</span>
      <button class="copy-single" data-original-index="${originalIndex}" aria-label="Copy issue: ${issue.title}">Copy</button>
    </div>
    <div class="issue-title" role="heading" aria-level="3">${escapeHtml(issue.title)}${outdatedLabel}${confidenceBadge}</div>
    <div class="issue-file" aria-label="File path">📁 ${escapeHtml(issue.filePath)}</div>
    <div class="issue-file" aria-label="Author: ${issue.author}">${authorBadge} ${escapeHtml(issue.author)}</div>
  `;
